...
var read = new Array(); // store the output value of r. ****
...
for(var term in addindex){  //******

//	if(i<100){
	label[i] = addindex[i][0];
	fid[i] = addindex[i][1];
	revid[i] = addindex[i][2];   //******
...
//	console.log(instance.address);
 ...
 //	return instance.storelabel(label,fid);
	...
	instance.updateadd(label_part[counter],fid_part[counter],revid_part[counter]).then(function(read_re){
	...
	instance.read_r_value.call().then(function(read_re){
	
	        read.push(read_re);
	        //console.log(read)
	        if (read[1] ==0)
	        {
	        console.log('Added Index Store Successfully');
	        //wordcount.writeFile('./local/db/dic/localdicjs.json', read, {flag: 'a'}, function(err){
	       // console.error(err);
	       // });
	        }
			...
			{
			console.log('The Added Index Already Exists');
			//wordcount.writeFile('./local/db/dic/localdicjs.json', read, {flag: 'a'}, function(err){
	        //console.error(err);
	        //});
			...
			console.log(read_re);
			console.log('Reading r from the contract!');
		}).catch(function (err){
			console.log(err);
		})
...
//	console.log(label_part);
    instance.updateadd(label_last,fid_last,revid_last).then(function(read_re){
		console.log(read_re);
	...
	instance.read_r_value.call().then(function(read_re){
			...
			{
			console.log('The Added Index Already Exists');
			//wordcount.writeFile('./local/db/dic/localdicjs.json', read, 'a', function(err){
	        //console.error(err);
	        //});
			...
			console.log(read_re);
			console.log('Reading r from the contract!');
			wordcount.writeFile('./local/db/dic/localdicjs.json', read, function(err){
	        console.error(err);
	        });
		}).catch(function (err){
			console.log(err);
